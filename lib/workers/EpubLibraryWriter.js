define(['storage/StorageManager', 'storage/EpubUnzipper', 'workers/Messages', 'workers/encryption_handler'], function(StorageManager, EpubUnzipper, Messages, EncryptionHandler){
	...
	LibraryWriter.prototype = {
        ...
        _addEpub : function(options, packageObj, packagePath, encryptionData){
            ...
            var encryptionHandler = new EncryptionHandler(encryptionData);
            ...
            var stubDecryptionFunction = function(data, callback) {
                callback(data);
            };
            ...
            for (var name in files){
                ...
                (function(name){
                    ...
                    decryptionFunction = encryptionHandler.getDecryptionFunctionForRelativePath(name);
                    if(!decryptionFunction) {
                        decryptionFunction = stubDecryptionFunction;
                    }
                    else {
                        console.debug(">>> Deobfuscation function was found for file " + name);
                    }

                    decryptionFunction(files[name], function(decryptedContent) {
                        StorageManager.saveFile(path, decryptedContent, function(){
