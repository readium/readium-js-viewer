define(['hgn!templates/managed-dialog.html', 'hgn!templates/progress-dialog.html', 'hgn!templates/managed-buttons.html', 'i18n/Strings', 'workers/Messages', 'keymaster', 'storage/Settings'], function(ManagedDialog, ProgressDialog, ButtonTemplate, Strings, Messages, key, Settings){
	var $currentModal,
		lastTitle;
...
// console.debug("PROP AFTER: " + e[_prop_name]);
	...
	Dialogs = {	
        ...
        keyboard:
        {
            resetToDefaults: function()
            {
                ...
                for (prop in Dialogs.keyboard.defaultOptions)
                {
                    if (!Dialogs.keyboard.defaultOptions.hasOwnProperty(prop)) continue;

                    if (typeof Dialogs.keyboard.defaultOptions[prop] !== 'string') continue;

                    Dialogs.keyboard[prop] = Dialogs.keyboard.defaultOptions[prop];
                }
            },
            resetAccessKeys: function()
            {
                ...
                Dialogs.keyboard.accesskeys = {};
                for (prop in Dialogs.keyboard)
                {
                    if (!Dialogs.keyboard.hasOwnProperty(prop)) continue;

                    var str = Dialogs.keyboard[prop];
    
                    if (typeof str !== 'string') continue;
    
                    Dialogs.keyboard.accesskeys[prop] = extractAccessKey(str);
                }
            },
            applySettings: function(json)
            {
                ...
                if (json && json.keyboard)
                {
                    ...
                    for (prop in Dialogs.keyboard)
                    {
                        ...
                        Dialogs.keyboard[prop] = json.keyboard[prop];
                    }
                }
    ...
    try
    {
        ...
        Dialogs.keyboard.defaultOptions = {};
        for (prop in Dialogs.keyboard)
        {
            if (!Dialogs.keyboard.hasOwnProperty(prop)) continue;
            ...
            Dialogs.keyboard.defaultOptions[prop] = Dialogs.keyboard[prop];
        }
        ...
        //unnecessary
        //Dialogs.keyboard.resetToDefaults();
        
        //necessary!
        Dialogs.keyboard.resetAccessKeys();
    }
    catch(e)
    {
        console.error(e);
