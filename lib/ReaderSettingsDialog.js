define(['hgn!templates/settings-dialog.html', 'i18n/Strings', 'Dialogs', 'storage/Settings'], function(SettingsDialog, Strings, Dialogs, Settings){
	var defaultSettings = {
        fontSize: 100,
	...
	var initDialog = function(reader){
		$('#app-container').append(SettingsDialog({strings: Strings, dialogs: Dialogs}));
        ...
        $('#keyboard-button').on('click', function(){
            ...
            if (hidden && hidden.length && hidden[0])
            {
                ...
                for (prop in Dialogs.keyboard)
                {
                    if (!Dialogs.keyboard.hasOwnProperty(prop)) continue;
                    
                    if (prop.indexOf(Dialogs.keyboardAccesskeyMark) >= 0) continue;
            
                    if (typeof Dialogs.keyboard[prop] !== 'string') continue;
                    
                    html += '<li>';
                    
                    html += '<label id="label_'+prop+'">';
                    html += Dialogs.keyboard.i18n[prop];
                    html += "<br/>";
                    html += '<span>';
                    html += prop;
                    html += "</span>";
                    ...
                    html += '<input id="'+prop+'" name="'+prop+'" type="text" tabindex="1000" placeholder="'+Dialogs.keyboard[prop]+'" value="'+Dialogs.keyboard[prop]+'" aria-labelledbyxxx="label_'+prop+'" aria-label="'+Dialogs.keyboard.i18n[prop]+'" title="'+Dialogs.keyboard.i18n[prop]+'"></input>';
                    html += '<button class="resetKey captureKeyboardShortcut" role="button" data-key="'+prop+'" tabindex="1000" title="'+Strings.i18n_reset_key+' ('+Dialogs.keyboard.defaultOptions[prop]+')" aria-label="'+Strings.i18n_reset_key+' ('+Dialogs.keyboard.defaultOptions[prop]+')"><span aria-hidden="true">&#8855;</span></button>';
                ...
                $(".resetKey").on("click", function()
                {
                    ...
                    if (id)
                    {
                        ...
                        $input.val(Dialogs.keyboard.defaultOptions[id]);
                    }
                    else
                    {
        ...
        var save = function(){
            ...
            Settings.get('reader', function(json)
            {
                ...
                $("#keyboard-list li input").each(function()
                {
                    ...
                    if (Dialogs.keyboard.defaultOptions[id] !== val)
                    {
                        // if (!json.keyboard)
                        // {
                        //     json.keyboard = {};
                ...
                setTimeout(function()
                {
                    Dialogs.keyboard.applySettingsAndResetAccessKeys();
                }, 100);
            });
        };
        
        Dialogs.keyboard.on(Dialogs.keyboard.NightTheme, 'settings', function(){
        ...
        Dialogs.keyboard.on(Dialogs.keyboard.SettingsModalSave, 'settings', function() {
            save();
            $('#settings-dialog').modal('hide');
        });

        Dialogs.keyboard.on(Dialogs.keyboard.SettingsModalClose, 'settings', function() {
            $('#settings-dialog').modal('hide');
        });
