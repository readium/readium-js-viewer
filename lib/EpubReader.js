...
Readium){
    ...
    var loadEbook = function (packageDocumentURL, readerSettings, openPageRequest) {
        ...
        readium.openPackageDocument(packageDocumentURL, function(packageDocument, options){
			...
			_multipleRenditions = options.multipleRenditions;
			SettingsDialog.updateMultipleRenditions(_multipleRenditions, _renditionSelection);

        }, openPageRequest, _renditionSelection);
    };
...
//console.debug("!! SPIN REQ: -- WILL: " + spinner.willSpin + " IS:" + spinner.isSpinning + " STOP REQ:" + spinner.stopRequested);
    ...
    var savePlace = function(){
		...
		if (_multipleRenditions) {
			var rendition = _multipleRenditions.renditions[_multipleRenditions.selectedIndex];
...
console.debug(bookMark);
        Settings.put(url, bookMark, $.noop);
    }
    ...
    var initReadium = function(){
        ...
        Settings.getMultiple(['reader', url], function(settings){
            ...
            if (settings[url]) {
                ...
                var bookmark = JSON.parse(settings[url]); // string
