...
//console.debug("SPIN: -- WILL: " + spinner.willSpin + " IS:" + spinner.isSpinning + " STOP REQ:" + spinner.stopRequested);
    ...
    var loadToc = function(dom){
        ...
        if (toc && toc.length)
        {
            ...
            // Not needed, jQuery already does that by default
            //$('script', $toc).remove();
            
            // These two can potentially execute base64-encoded ecmascript via image/svg served as data:URI (with AllowScriptAccess),
            // but the web browser's "data:" vs. "http:"/"filesystem:"/"file:" origin/domain restrictions
            // already protect against script injection into the container domain (window.top / window.parent)
            // $('embed', $toc).remove();
            // $('object', $toc).remove();
            
            // This one really needs to be nuked! (src="javascript:doHorribleThings(window.top)")
            $('iframe', $toc).remove();
