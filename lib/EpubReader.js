    ...
    var initReadium = function(){
        ...
        Settings.getMultiple(['reader', url], function(settings){
            ...
            Versioning.getVersioningInfo(function(version){
                ...
                window.navigator.epubReadingSystem.readium.buildInfo.releaseTagNameHelp = "Tip: to obtain the tag/release name relative to a git commit SHA, use the command line: 'git name-rev --tags --name-only SHA' => returns: TAG_NAME^n or TAG_NAME~n (where 'n' is the number of revisions ahead / behind)";
