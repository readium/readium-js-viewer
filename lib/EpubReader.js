...
//console.debug("!! SPIN REQ: -- WILL: " + spinner.willSpin + " IS:" + spinner.isSpinning + " STOP REQ:" + spinner.stopRequested);
    ...
    var toggleFullScreenIcon = function(){
        ...
        var ariaTitleAttrValue = Strings.enter_fullscreen + " ["+Keyboard.FullScreenToggle+"]";

        if(screenfull.isFullscreen)
        {
            ariaTitleAttrValue = Strings.exit_fullscreen + " ["+Keyboard.FullScreenToggle+"]";
            //toggle glyphicon
            $("#buttFullScreen > :first-child").removeClass('glyphicon-resize-full').addClass('glyphicon-resize-small');
        }
        else
            $("#buttFullScreen > :first-child").removeClass('glyphicon-resize-small').addClass('glyphicon-resize-full');

        $('#buttFullScreen').attr( 'aria-label', ariaTitleAttrValue).attr('title', ariaTitleAttrValue);
    }
    ...
    var installReaderEventHandlers = function(){
        ...
        $('#buttFullScreen').on('click', toggleFullScreen);

        if(screenfull.enabled)
            screenfull.onchange = function(){toggleFullScreenIcon();};
