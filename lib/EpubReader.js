...
define(['module','jquery', 'bootstrap', 'URIjs', 'Readium', 'Spinner', 'storage/Settings', 'i18n/Strings', 'ReaderSettingsDialog', 
        'hgn!templates/about-dialog.html', 'hgn!templates/reader-navbar.html', 'hgn!templates/reader-body.html', 'analytics/Analytics', 'keymaster'], 
        function (module, $, bootstrap, URI, Readium, spinner, Settings, Strings, SettingsDialog, AboutDialog, ReaderNavbar, ReaderBody, Analytics, key) {
...
// console.debug("PROP AFTER: " + e[_prop_name]);
    ...
    {
        key.unbind('ctrl+alt+s', 'settings');
        key.unbind('ctrl+alt+c', 'settings');

        key.unbind('ctrl+alt+n', 'reader');
        key.unbind('ctrl+alt+x', 'reader');
        key.unbind('ctrl+alt+v', 'reader');
        key.unbind('ctrl+alt+1', 'reader');
        key.unbind('ctrl+alt+2', 'reader');
        key.unbind('ctrl+alt+o', 'reader');
        key.unbind('ctrl+alt+f', 'reader');
        key.unbind('ctrl+alt+b', 'reader');
        key.unbind('ctrl+alt+t', 'reader');
        key.unbind('ctrl+alt+m', 'reader');
        key.unbind('ctrl+alt+k', 'reader');
        key.unbind('ctrl+alt+a', 'reader');
    };
    ...
    var installReaderEventHandlers = function(){
        ...
        {
            return document.activeElement && (jQuery.contains(document.getElementById("app-navbar"), document.activeElement) || jQuery.contains(document.getElementById("settings-dialog"), document.activeElement) || jQuery.contains(document.getElementById("about-dialog"), document.activeElement));  
        };

        key('left', 'reader', function(){
            if (!isWithinForbiddenNavKeysArea()) prevPage();
        });
        key('right', 'reader', function(){
            if (!isWithinForbiddenNavKeysArea()) nextPage();
        });


        key('ctrl+alt+x', 'reader', function(){
            $(document.body).addClass('hide-ui');
        });
        key('ctrl+alt+v', 'reader', function(){
            $(document.body).removeClass('hide-ui');
        });

        key('ctrl+alt+1', 'reader', prevPage);
        $("#previous-page-btn").unbind("click");
        $("#previous-page-btn").on("click", prevPage);

        key('ctrl+alt+2', 'reader', nextPage);
        ...
        key('ctrl+alt+f', 'reader', toggleFullScreen);
        ...
        key('ctrl+alt+b', 'reader', loadlibrary /* function(){setTimeout(, 30);} */ );
        $('.icon-library').on('click', function(){
            loadlibrary();
            return false;
        });

        key('ctrl+alt+t', 'reader', tocShowHideToggle);
        ...
        key('ctrl+alt+o', 'reader', function(){$('#settings-dialog').modal("show")});
...
// newE.originalEvent.view = window;
            ...
            readium.reader.addIFrameEventListener('focus', function(e) {
                //window.dispatchEvent(document.createEvent("Event"));
            ...
            key('ctrl+alt+n', 'reader', function(){
                if (!embedded){
    ...
    var initMediaOverlay = function(){
        ...
        key('ctrl+alt+escape', 'reader', readium.reader.escapeMediaOverlay);
        ...
        key('ctrl+alt+m, ctrl+alt+space', 'reader', readium.reader.toggleMediaOverlay);
        ...
        var changeRate = function(minus)
        {
            ...
            $changeRateControl.attr("aria-valuenow", ""+rateVal);
            $changeRateControl.attr("aria-value-now", ""+rateVal);
            
            $changeRateControl.attr("aria-valuetext", txt);
            $changeRateControl.attr("aria-value-text", txt);
        ...
        key('ctrl+alt+l', 'reader', function(){        
            changeRate(false);
        });
        key('ctrl+alt+j', 'reader', function(){
            changeRate(true);
        });
        ...
        key('ctrl+alt+k', 'reader', resetRate);
        ...
        key('ctrl+alt+w', 'reader', function(){
            changeVolume(false);
        });
        
        key('ctrl+alt+q', 'reader', function(){
            changeVolume(true);
        });
        ...
        key('ctrl+alt+a', 'reader', function(){
        ...
        key('ctrl+alt+[, ctrl+alt+{', 'reader', readium.reader.previousMediaOverlay);
        $previousAudioBtn.on("click", readium.reader.previousMediaOverlay);
        
        key('ctrl+alt+], ctrl+alt+}', 'reader', readium.reader.nextMediaOverlay);
