...
define(['module','jquery', 'bootstrap', 'URIjs', 'Readium', 'Spinner', 'storage/Settings', 'i18n/Strings', 'ReaderSettingsDialog', 
    ...
    var initReadium = function(){
        ...
        Settings.getMultiple(['reader', url], function(settings){
            ...
            var mediaOverlaysHasSync_word = false;
            var mediaOverlaysHasSync_sentence = false;
            var mediaOverlaysHasSync_paragraph = false;
            ...
            readium.reader.on(ReadiumSDK.Events.PAGINATION_CHANGED, function (pageChangeData) {
                ...
                if (!mediaOverlaysHasSync_word && !readium.reader.mediaOverlaysHasSync("word"))
                {
                    $moSyncWord.attr('disabled', "disabled");
                    mediaOverlaysHasSync_word = true;
                }
                else
                {
                ...
                if (!mediaOverlaysHasSync_sentence && !readium.reader.mediaOverlaysHasSync("sentence"))
                {
                    $moSyncSentence.attr('disabled', "disabled");
                    mediaOverlaysHasSync_sentence = true;
                }
                else
                {
                ...
                if (!mediaOverlaysHasSync_paragraph && !readium.reader.mediaOverlaysHasSync("paragraph"))
                {
                    $moSyncParagraph.attr('disabled', "disabled");
                    mediaOverlaysHasSync_paragraph = true;
                }
                else
                {
            ...
            readium.reader.on(ReadiumSDK.Events.CONTENT_DOCUMENT_LOADED, function ($iframe, spineItem) {
                ...
                mediaOverlaysHasSync_word = false;
                mediaOverlaysHasSync_sentence = false;
                mediaOverlaysHasSync_paragraph = false;
            });
