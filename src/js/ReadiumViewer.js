define(['jquery', './EpubLibrary', './EpubReader', 'readium_shared_js/helpers', './ModuleConfig', './EpubFilesystemUnzip', 'URIjs'], function($, EpubLibrary, EpubReader, Helpers, moduleConfig, EpubFilesystemUnzip, URI){
            ...
            if (files.length) {
                ...
                if (isChromeExtensionPackagedApp) {
                    ...
                    if (file.type == "application/epub+zip" || (/\.epub[3?]$/.test(file.name))) {
						...
						EpubFilesystemUnzip.explode(file,
							//SUCCESS
							// epub parameter should be the filesystem: URL
							function(epub) {
								console.log("EPUB archive succesfully exploded into local HTML filesystem.");
								launch(epub);
							},
							// FAILURE
							// let's just use the file Blob
							function() {
								console.log("EPUB archive cannot be exploded into local HTML filesystem. Reading directly from zipped ebook.");
								launch(file);
							}
						);
                    }
